...
function my_onReceive(swoole_server $serv, $fd, $from_id, $data)
{
    ...
    if ($cmd->ACTION == "reload") {
        $serv->reload();
    } elseif ($cmd->ACTION == "GET") {
        $cmd->FD = $fd;
        $task_id = $serv->task($cmd);
...
function my_onTask(swoole_server $serv, $task_id, $from_id, $data)
{
    ...
    if ($data == 'taskwait') {
        ...
        if ($data->ACTION == "GET") {
            ...
            $serv->send($data->FD, json_encode($user));
            var_dump($sql);
            var_dump($user);
        } else {
            $sql = "Insert into `message`(`content`, `createtime`,`fromusername`) values('adfadfga','{9874864}', '{$fd}')";
            if ($mysql->query($sql)) {
                echo "Insert Success";
            }
